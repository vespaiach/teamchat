...
class ApplicationController < ActionController::Base
  ...
  def authenticate_with_remember_token
    return unless cookies.signed[:_teamchat_remember_token]

    user = User.find_by(remember_token: cookies.signed[:_teamchat_remember_token])

    if user&.remember_token_valid?
      session[:user_id] = user.id
      # Rotate token for security
      user.generate_remember_token!
      cookies.permanent.signed[:_teamchat_remember_token] = user.remember_token
      user
    else
